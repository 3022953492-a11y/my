...
本项目实现了一个基于图流加序列流的化学公式预测系统，能够从化学反应式图像中识别化学公式并生成准确的化学方程式表示。系统结合了图神经网络和序列模型的双流特征提取能力，通过交叉注意力机制实现特征融合，最终输出格式规范的化学方程式。
...
#### 示例1: 化学反应式 (NaOH + HCl = NaCl + H₂O)

| 阶段 | 预测结果 | 说明 |
|------|----------|------|
| **输入图像** | ![NaOH+HCl图像](./dataset/images/1.jpg) | 化学反应式图像 |
| **序列流预测** | `NaOH + HCl = NaCl + H2O` | 仅基于图像特征的预测 |
| **图流预测** | `NaOH + HCl = NaCl + H₂O` | 基于化学公式图结构的预测 |
| **图流+序列流预测** | `NaOH + HCl = NaCl + H₂O` | 双流融合的预测结果 |
| **CRF后处理** | `NaOH + HCl = NaCl + H₂O` ✓ | 应用化学语法约束的最终输出 |

#### 示例2: 化学反应式 (2H₂ + O₂ = 2H₂O)

| 阶段 | 预测结果 | 说明 |
|------|----------|------|
| **输入图像** | ![2H2+O2图像](./dataset/images/2.jpg) | 化学反应式图像 |
| **序列流预测** | `2H2 + O2 = 2H2O` | 仅基于图像特征的预测 |
| **图流预测** | `2H₂ + O₂ = 2H₂O` | 基于化学公式图结构的预测 |
| **图流+序列流预测** | `2H₂ + O₂ = 2H₂O` | 双流融合的预测结果 |
| **CRF后处理** | `2H₂ + O₂ = 2H₂O` ✓ | 应用化学语法约束的最终输出 |

#### 示例3: 化学反应式 (H₂SO₄ + 2NaOH = Na₂SO₄ + 2H₂O)

| 阶段 | 预测结果 | 说明 |
|------|----------|------|
| **输入图像** | ![H2SO4+NaOH图像](./dataset/images/3.jpg) | 化学反应式图像 |
| **序列流预测** | `H2SO4 + 2NaOH = Na2SO4 + 2H2O` | 仅基于图像特征的预测 |
| **图流预测** | `H₂SO₄ + 2NaOH = Na₂SO₄ + 2H₂O` | 基于化学公式图结构的预测 |
| **图流+序列流预测** | `H₂SO₄ + 2NaOH = Na₂SO₄ + 2H₂O` | 双流融合的预测结果 |
| **CRF后处理** | `H₂SO₄ + 2NaOH = Na₂SO₄ + 2H₂O` ✓ | 应用化学语法约束的最终输出 |

#### 示例4: 复杂化学反应式 (CaCO₃ + 2HCl = CaCl₂ + H₂O + CO₂)

| 阶段 | 预测结果 | 说明 |
|------|----------|------|
| **输入图像** | ![CaCO3+HCl图像](./dataset/images/4.jpg) | 化学反应式图像 |
| **序列流预测** | `CaCO3 + 2HCl = CaCl2 + H2O + CO2` | 仅基于图像特征的预测 |
| **图流预测** | `CaCO₃ + 2HCl = CaCl₂ + H₂O + CO₂` | 基于化学公式图结构的预测 |
| **图流+序列流预测** | `CaCO₃ + 2HCl = CaCl₂ + H₂O + CO₂` | 双流融合的预测结果 |
| **CRF后处理** | `CaCO₃ + 2HCl = CaCl₂ + H₂O + CO₂` ✓ | 应用化学语法约束的最终输出 |
